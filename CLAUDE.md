...
- **Deploy:** Vercel
- **Newsletter:** tabla `subscribers` en Supabase; alta con doble opt-in vía **Kit** ya integrada (`lib/kit.ts`). Kit manda el correo de verificación; un webhook (`app/api/kit/webhook/route.ts`) marca `confirmed = true`
...
- **Newsletter con Kit (doble opt-in)** — flujo completo, verificado en local (falta prueba end-to-end del correo real de Kit + webhook, que necesitan deploy):
  ...
  - **Kit** (`lib/kit.ts`): `addToKit` postea al **endpoint público de submission del Form** (`https://app.kit.com/forms/{KIT_FORM_ID}/subscriptions`, form-encoded, el mismo que usa el embed) — es lo único que dispara el doble opt-in y el correo de verificación. **La API v4 NO sirve para el opt-in:** `POST /v4/forms/{id}/subscribers` da 404 y `POST /v4/subscribers` crea al subscriber como `active` sin mandar confirmación (hasta setear `state:"inactive"` está deshabilitado). No volver a intentar por v4. `addToKit` es no-op sin `KIT_FORM_ID` (no usa la API key). `removeFromKit` (baja al eliminar) sí usa v4 (`KIT_API_KEY`): busca el id por correo y lo desuscribe.
