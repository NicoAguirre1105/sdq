...
- **Shell del admin**: `app/admin/layout.tsx` (protegido) + `components/admin/Sidebar.tsx` (client, `usePathname` para estado activo). El sidebar lista las 6 áreas; **Dashboard, Posts y Fútbol** están habilitados, el resto (Tienda, Suscriptores, Plantilla) se muestra deshabilitado con "pronto" (no enlazan a rutas 404 — habilitar a medida que se construyan). Logout en el footer del sidebar.
