...
- **Newsletter con Kit (doble opt-in)** — flujo completo, verificado en local (falta prueba end-to-end del correo real de Kit + webhook, que necesitan deploy):
  ...
  - **Kit** (`lib/kit.ts`): `addToKit`/`removeFromKit`. **No-op sin `KIT_API_KEY`/`KIT_FORM_ID`** (el flujo local funciona igual, queda pendiente en Supabase). El doble opt-in lo hace el Form de Kit (setting del form, no la API).
